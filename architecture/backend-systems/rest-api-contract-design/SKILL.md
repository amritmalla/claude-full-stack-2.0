...
`openapi.yaml` and `api-conventions.md` MUST conform to [standards/api-standards](../../../standards/api-standards/README.md). That standard is authoritative for:

- REST conventions: resource modeling, HTTP verbs, status codes.
- Versioning, cursor-based pagination, error envelope shape, rate-limit declaration.
- OpenAPI 3.1 requirement, spec location, lint gate.
- Auth requirements (see also [security-standards](../../../standards/security-standards/README.md)).

`api-conventions.md` is the per-service applied version of the standard — it records the specific choices the standard intentionally leaves open (PATCH semantics, error-code registry, auth scopes, deprecation notes).

Naming for paths, identifiers, and event topics conforms to [naming-conventions](../../../standards/naming-conventions/README.md).

Use `assets/openapi-starter.template.yaml` and `assets/api-conventions.template.md` as the scaffolds — they implement the standard.
...
- [ ] Step 12: Validate the contract against [standards/api-standards](../../../standards/api-standards/README.md) (error envelope shape, cursor pagination shape, versioning rule, rate-limit declaration, OpenAPI 3.1, spec location) AND against `references/api-contract-quality-rubric.md`. Revise until both pass or explicitly document any unresolved gap.
...
- [ ] Spec validates against [standards/api-standards](../../../standards/api-standards/README.md): error envelope matches the standard shape; pagination is cursor-based with the standard envelope; rate limits declared via `x-rate-limit` where relevant; auth scheme matches `security-standards`.
- [ ] `openapi.yaml` follows `assets/openapi-starter.template.yaml`.
- [ ] `api-conventions.md` follows `assets/api-conventions.template.md` and records every per-service choice the standard leaves open.
