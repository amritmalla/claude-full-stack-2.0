...
- Recognize PRDs that legally omit sections (Why Now, Current Alternatives, Risks, Distribution) under `idea-development` conditional-section rules. Do not flag legitimate omissions as PRD incompleteness; only flag missing decisions the design actually needs (primary user, core workflow, hard constraints).
