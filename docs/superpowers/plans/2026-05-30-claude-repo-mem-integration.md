# claude-full-stack-2.0 ↔ claude-repo-mem Integration Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans. Steps use checkbox (`- [ ]`) syntax.

**Goal:** Make claude-repo-mem the recommended memory layer for projects built using claude-full-stack-2.0's 82 skills. Ship (a) an MCP server config so claude-repo-mem runs alongside, and (b) a new companion skill `memory-management` documenting *when* skills should call `recall` / `remember` / `handoff` / `resume`.

**Architecture:** claude-repo-mem ships separately (its own PyPI package + MCP stdio server). claude-full-stack-2.0 doesn't bundle it — instead, it provides a turnkey `.mcp.json`, bootstrap docs, and one new skill that other skills are *encouraged* (not required) to reference. No existing SKILL.md files are modified. The 4 WORKFLOW.md files are not modified in this pass either — they can adopt the skill later.
...
**Spec sources:**
- claude-repo-mem: `D:/projects/claude-full-stack-mem/docs/specs/2026-05-25-claude-repo-mem-design.md`
- claude-full-stack-2.0 SKILL spec: `D:/projects/claude-full-stack-2.0/SKILL_SPEC.md`

**Constraints:**
- claude-repo-mem is invoked from outside this repo (the *user's* repo, where work happens).
- The MCP config shipped here is a *template* — users copy it into their workspace because claude-repo-mem indexes a single repo at a time.
...
2. Put it under implementations: `skills/implementations/tooling/claude-repo-mem/SKILL.md` (tooling category doesn't currently exist).
...
claude-repo-mem's server runs over stdio. The template `.mcp.json` users drop in their workspace:
...
If `claude-repo-mem` isn't on PATH, users can swap `command` for the venv path. The setup reference covers both.
...
> Use when a project built with claude-full-stack-2.0 needs durable cross-session memory, retrieval over a repo's code/docs/decisions, or task handoff between sessions. Defines how to install and configure the claude-repo-mem MCP server and when to call its tools (recall, trace, remember, forget, plan_task, tasks, handoff, resume) during architecture, implementation, and operations work.
...
- **Inputs** — claude-repo-mem version, OPENAI/VOYAGE keys if hosted embeddings desired, target repo path.
...
- **Steps** — install, configure MCP, run `claude-repo-mem index`, verify with `doctor`, optionally `install-hooks`.
- **Outputs** — a working MCP server, a populated `.claude-repo-mem/` directory in the user's workspace.
- **Definition of done** — `recall` returns ranked results for a known symbol; `remember` writes a markdown file under `.claude-repo-mem/memory/<scope>/`.
...
description: Use when a project built with claude-full-stack-2.0 needs durable cross-session memory, retrieval over a repo's code/docs/decisions, or task handoff between sessions. Sets up the claude-repo-mem MCP server and prescribes when to call recall, trace, remember, forget, plan_task, tasks, handoff, and resume during architecture, implementation, and operations work.
```
...
Walkthrough: install via pip, copy `.mcp.json` into user workspace, run `claude-repo-mem index`, verify with `claude-repo-mem doctor`, optional `claude-repo-mem install-hooks`, switching embedders. Include Windows + macOS/Linux notes.

- [ ] **Step 3: Write `references/usage-by-phase.md`**

Maps claude-repo-mem tools to claude-full-stack-2.0 workflow phases:
...
git commit -m "feat(skills): memory-management — companion skill for claude-repo-mem MCP integration"
```
...
claude-repo-mem is a local-first MCP server that gives Claude durable memory and
...
between claude-full-stack-2.0 workflow phases and claude-repo-mem tools.
...
255 tests passing. Source: <https://github.com/amritmalla/claude-repo-mem>.
```
...
git commit -m "docs: INTEGRATION.md — claude-repo-mem setup pointer at repo root"
```
...
[claude-repo-mem](https://github.com/amritmalla/claude-repo-mem) and follow
[`INTEGRATION.md`](./INTEGRATION.md). The `memory-management` skill defines
when each claude-repo-mem tool is appropriate during architecture, implementation,
and operations work.
```
...
```
git add README.md
git commit -m "docs: README — point to claude-repo-mem integration"
```
...
- claude-repo-mem is NOT bundled — users install separately via pip.
- Other potential integrations (e.g., embedded claude-repo-mem in the marketplace, vendored Python package) are out of scope.
