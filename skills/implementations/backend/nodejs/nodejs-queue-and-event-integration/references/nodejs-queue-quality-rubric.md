# Node.js Queue and Event Integration Quality Rubric

Load this before declaring the queue/event integration complete. Revise until each check passes or the unresolved gap is explicitly documented in the service README.
...
- [ ] Producer, consumer, and relay are registered in the scaffold DI container.
- [ ] Broker connection settings resolve through the scaffold validated config seam.
- [ ] Produce/consume logging goes through the scaffold logger (no second logger).
- [ ] Consumer stop/drain is wired into the scaffold graceful-shutdown hook.
...
- [ ] Every consumer is idempotent via an explicit dedupe-key store (message id or business key).
...
- [ ] On `SIGTERM` the consumer stops fetching new messages.
- [ ] In-flight messages drain within the scaffold shutdown timeout.
- [ ] Incomplete work is not acked; it redelivers (test-verified).

## Envelope and schema

- [ ] Messages use a versioned envelope (`id`, `type`, `occurredAt`, `schemaVersion`, `payload`).
- [ ] Each message type has a zod schema validated on both produce and consume.
...
- [ ] Metric hooks are exposed for `nodejs-observability-readiness`; no OTel/prom vendor wired here.
...
3. Revise, then re-run `tsc --noEmit`, lint, the integration tests, and the boot smoke check.
