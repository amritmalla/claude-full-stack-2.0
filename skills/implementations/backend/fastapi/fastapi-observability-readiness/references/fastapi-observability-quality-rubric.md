# FastAPI Observability Readiness Quality Rubric
...
- [ ] The OpenTelemetry Python SDK is initialized with resource attributes and an OTLP exporter.
- [ ] Auto-instrumentation covers FastAPI/ASGI and the DB/client libraries.
- [ ] The SDK initializes during ASGI-lifespan startup (the app is instrumented).
...
- [ ] `/metrics` is exposed (or a push exporter is configured) per the settings mode.
...
- [ ] Correlation extends the scaffold structlog config and request context — no second logger.
...
- [ ] OTLP endpoint, sampling, and exposure mode resolve through the scaffold validated settings seam.
...
- [ ] [observability-standards](../../../../../../standards/observability-standards/README.md): three correlated signals; RED metrics with documented buckets; SLIs as queries; multi-burn-rate alerts tied to an error budget; bounded cardinality; settings-driven fail-safe export.
...
3. Revise, then re-run `mypy`, `ruff check`, tests, the boot smoke check, and the local three-signal correlation check.
