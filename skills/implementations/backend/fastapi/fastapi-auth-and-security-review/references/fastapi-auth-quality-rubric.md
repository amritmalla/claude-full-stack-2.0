# FastAPI Auth and Security Review Quality Rubric
...
- [ ] The verification strategy matches the model in `architecture/security` (OAuth2/OIDC / JWT-JWKS / API-key).
- [ ] The scaffold principal `Depends` seam is filled with a typed `Principal`; no auth shell TODO remains.
...
- [ ] An authorization error denies access (fails closed) — no except-into-allow path.

## HTTP surface hardening

- [ ] A security-headers middleware sets a deny-by-default CSP and HSTS.
- [ ] The server banner is removed.
...
- [ ] Every auth secret resolves through the scaffold validated settings seam (missing one aborts boot).
- [ ] No secret is hardcoded, logged, or committed; `.env.example` has placeholders only.
- [ ] `authorization`, `cookie`, token, and password keys are redacted in log output.
- [ ] Every external input is validated at the boundary with a Pydantic model; unknown fields are rejected (`extra="forbid"`).
...
- [ ] Error responses leak no traceback or internal detail outside dev.
- [ ] `pip-audit` (or equivalent) was run; high-severity advisories are fixed or explicitly accepted with rationale.
...
- [ ] The security suite runs as part of the standard `pytest` command.
...
- [ ] [security-standards](../../../../../../standards/security-standards/README.md): default-deny authorization, secrets via settings seam, secure headers, boundary validation, no secrets in source/image.
...
3. Revise, then re-run `mypy`, `ruff check`, the full test suite, and the boot smoke check.
