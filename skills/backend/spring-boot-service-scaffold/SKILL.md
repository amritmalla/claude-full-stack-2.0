...
- Confirm the target directory before writing files. Recommend `services/<service-name>/` in a monorepo or repo root for a single-service repo. Refuse to write into a plugin/skill repository (any directory containing `skills/`, `marketplace.json`, or this skill's own tree) without explicit user override.
...
- [ ] Step 10: Validate the scaffold against `references/scaffold-quality-rubric.md`. Revise until checks pass or explicitly document any unresolved gap.
