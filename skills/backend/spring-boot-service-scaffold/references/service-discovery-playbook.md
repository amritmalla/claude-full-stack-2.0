...
Recommend `services/<service-name>/` for monorepos and the repository root for single-service repos. Refuse to scaffold into any directory that looks like a plugin or skill repository (contains `skills/`, `marketplace.json`, `.claude-plugin/`, or this skill's own tree) without an explicit user override.
